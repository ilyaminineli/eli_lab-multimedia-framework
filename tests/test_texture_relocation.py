...
def test_plan_only_includes_actually_referenced_outside_texture(
    ...
    refs = [
        BlenderReference(
            Path("Scenes/Forest.blend"),
            Path("Scenes/textures/moss.png"),
            "image",
            "resolved",
        ),
        BlenderReference(
            Path("Scenes/Forest.blend"),
            Path("Assets/Textures/kept.png"),
            "image",
            "resolved",
        ),
    ]
    monkeypatch.setattr(
        relocation, "inspect_project", lambda root, blender_executable=None: refs
    )

    candidates = plan_referenced_texture_relocations(
        tmp_path, blender_executable="blender"
    )
...
def test_relocation_repairs_then_moves_and_creates_backup(
    tmp_path: Path, monkeypatch
) -> None:
    ...
    candidate = TextureRelocationCandidate(
        ...
        references=(
            BlenderReference(
                Path("Scenes/Forest.blend"), Path("old/moss.png"), "image", "resolved"
            ),
        ),
        reason="test",
        safe=True,
    ...
    monkeypatch.setattr(
        relocation,
        "_repair_blend",
        lambda blend_file, old_path, new_path, blender_executable: True,
    )
    monkeypatch.setattr(relocation, "record_history", lambda *args, **kwargs: None)

    result = relocate_texture_candidate(
        tmp_path, candidate, blender_executable="blender"
    )
    assert result.destination == Path("Assets/Textures/moss.png")
    assert not source.exists()
    assert (tmp_path / "Assets" / "Textures" / "moss.png").read_bytes() == b"png"
    assert (
        result.backup_directory / "Scenes" / "Forest.blend"
    ).read_bytes() == b"blend"
    assert (result.backup_directory / "relocation.json").exists()


def test_relocation_rolls_back_when_a_reference_cannot_be_repaired(
    tmp_path: Path, monkeypatch
) -> None:
    source = tmp_path / "old" / "moss.png"
    source.parent.mkdir(parents=True)
