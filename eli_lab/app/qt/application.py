...
QLineEdit, QPlainTextEdit, QTextBrowser, QTreeWidget, QTableWidget, QComboBox, QDateEdit, QSpinBox {
    ...
    border-radius: 6px;
    padding: 7px 9px;
    selection-background-color: #46566f;
...
def configure_palette(app: QApplication) -> None:
    """Set a real dark palette so native dialogs/popups do not fall back to white."""
    palette = QPalette()
    palette.setColor(QPalette.Window, "#202329")
    palette.setColor(QPalette.WindowText, "#e8eaed")
    palette.setColor(QPalette.Base, "#2a2f37")
    palette.setColor(QPalette.AlternateBase, "#252a31")
    palette.setColor(QPalette.Text, "#f3f4f6")
    palette.setColor(QPalette.Button, "#353b45")
    palette.setColor(QPalette.ButtonText, "#f5f6f7")
    palette.setColor(QPalette.Highlight, "#46566f")
    palette.setColor(QPalette.HighlightedText, "#ffffff")
    palette.setColor(QPalette.ToolTipBase, "#252a31")
    palette.setColor(QPalette.ToolTipText, "#f3f4f6")
    palette.setColor(QPalette.PlaceholderText, "#747d89")
...
class MainWindow(QMainWindow):
    ...
    def __init__(self) -> None:
        ...
        self.navigation.setIndentation(14)
        self.navigation.setUniformRowHeights(False)
        self._tool_rows: dict[str, int] = {}
        self._build()
        self.navigation.itemClicked.connect(self._select_tool)
    ...
    def _build(self) -> None:
        ...
        for index, tool in enumerate(TOOLS):
            ...
            if category_item is None:
                ...
                category_item.setFlags(category_item.flags() & ~category_item.flags().__class__.ItemIsSelectable)
                category_item.setForeground(0, self.navigation.palette().color(QPalette.Disabled, QPalette.Text))
                self.navigation.addTopLevelItem(category_item)
                categories[tool.category] = category_item

            child = QTreeWidgetItem([tool.name])
            child.setData(0, 32, index)
            child.setToolTip(0, tool.description)
            category_item.addChild(child)
            self._tool_rows[tool.key] = index
            self.stack.addWidget(self._make_tool_page(tool.name, tool.category, tool.description, tool.key))

        for category_item in categories.values():
            category_item.setExpanded(True)
            category_item.setDisabled(False)

        self.navigation.setCurrentItem(self.navigation.topLevelItem(0).child(0))
        ...
        if self.navigation.topLevelItemCount():
            first = self.navigation.topLevelItem(0).child(0)
            self.navigation.setCurrentItem(first)
            self._select_tool(first, 0)

    def _make_tool_page(self, name: str, category: str, description: str, key: str) -> QWidget:
