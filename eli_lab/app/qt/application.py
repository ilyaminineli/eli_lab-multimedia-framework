...
class MainWindow(QMainWindow):
    ...
    def _build(self) -> None:
        ...
        for index, tool in enumerate(TOOLS):
            ...
            if category is None:
                ...
                categories[tool.category] = category
    ...
    def _make_tool_page(self, key: str) -> QWidget:
        if key == "workspace":
            tool = WorkspaceTool(self)
        elif key == "audit":
            tool = AuditTool(self)
        elif key == "material_library":
            tool = MaterialLibraryTool(self)
        elif key == "texture_relocation":
            tool = TextureRelocationTool(self)
        else:
    ...
    def _filter_tools(self, query: str) -> None:
        ...
        for i in range(self.navigation.topLevelItemCount()):
            category = self.navigation.topLevelItem(i)
            visible_count = 0
            for j in range(category.childCount()):
                ...
                visible = not needle or (
                    tool
                    and needle
                    in f"{tool.name} {tool.category} {tool.description}".casefold()
                )
                child.setHidden(not visible)
                visible_count += int(visible)
            category.setHidden(visible_count == 0)
            if needle and visible_count:
                category.setExpanded(True)
