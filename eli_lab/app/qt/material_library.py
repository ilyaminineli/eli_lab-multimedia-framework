...
from eli_lab.production.materials import discover_material_records, plan_texture_relocation
...
class MaterialLibraryTool(ToolWidget):
    ...
    def scan(self) -> None:
        ...
        for row, record in enumerate(records):
            ...
            self.table.setItem(row, 1, QTableWidgetItem(", ".join(str(path) for path in record.source_files)))
            self.table.setItem(row, 2, QTableWidgetItem(", ".join(record.texture_sets) or "—"))
        self.summary.setText(f"{len(records)} material source(s) detected.")
        if relocations:
            examples = "\n".join(f"{source}  →  {destination}" for source, destination in relocations[:12])
            suffix = f"\n…and {len(relocations) - 12} more." if len(relocations) > 12 else ""
            self.misplaced.setText(f"Misplaced textures: {len(relocations)}\n{examples}{suffix}\n\nRelocation is intentionally planned separately because Blender references must be repaired before files are moved.")
        else:
