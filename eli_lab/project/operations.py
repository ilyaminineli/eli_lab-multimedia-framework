...
from .workspace import EntityMetadata, entity_path, record_history, save_entity_metadata, scan_workspace


def create_entity(root: str | Path, kind: str, name: str, *, subfolders: tuple[str, ...] = ()) -> ProjectEntity:
    ...
    entity = next(item for item in discover_entities(root_path) if item.path == path.relative_to(root_path))
    write_entity_markdown(root_path, entity)
    record_history(root_path, "create_entity", entity.relative_path, f"kind={kind}")
    return entity


def add_entity_to_standard_project(root: str | Path, kind: str, name: str) -> ProjectEntity:
...
def refresh_project_documentation(root: str | Path) -> Path:
    ...
    record_history(summary.root, "refresh_documentation", output.name, f"entities={len(summary.entities)}")
    return output


def regenerate_structure(root: str | Path, project_name: str, *, structure=None) -> list[Path]:
