...
class WorkspaceSummary:
    ...
    def counts(self) -> dict[str, int]:
        return {
            kind: len(items) for kind, items in group_entities(self.entities).items()
        }
...
def load_entity_metadata(entity_path: str | Path) -> EntityMetadata:
    ...
    if not path.exists():
        return EntityMetadata(
            name=target.stem if target.is_file() else target.name, kind="asset"
        )
    data = json.loads(path.read_text(encoding="utf-8"))
    defaults = asdict(EntityMetadata("", "asset"))
    return EntityMetadata(
        **{field: data.get(field, default) for field, default in defaults.items()}
    )
...
def save_entity_metadata(entity_path: str | Path, metadata: EntityMetadata) -> Path:
    ...
    path.write_text(
        json.dumps(asdict(metadata), indent=4, ensure_ascii=False) + "\n",
        encoding="utf-8",
    )
    return path
...
def record_history(
    root: str | Path, operation: str, target: str = "", details: str = ""
) -> Path:
...
def scan_workspace(root: str | Path) -> WorkspaceSummary:
    """Read an existing project without requiring it to have been created by eli_lab."""
    ...
    return WorkspaceSummary(
        root_path, metadata, entities, discover_project_files(root_path)
    )
