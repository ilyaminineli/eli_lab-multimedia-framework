...
def scan_project(root: str | Path) -> ProjectScan:
    ...
    loose_blends = [
        p
        for p in files
        if p.suffix.casefold() == ".blend"
        and (
            len(p.parts) == 1
            or (len(p.parts) == 2 and p.parts[0].casefold() == "scenes")
        )
    ]
    loose_textures = [
        p for p in files if p.suffix.casefold() in TEXTURES and len(p.parts) == 1
    ]
    uuid_dirs: list[Path] = []
    scene_asset_dirs: list[Path] = []
    for p in root.rglob("*"):
        ...
        if (
            p.is_dir()
            and _uuidish(p.name)
            and p.parent.name.casefold()
            in {"models", "materials", "textures", "hdrs", "scenes"}
        ):
            uuid_dirs.append(p.relative_to(root))
        if (
            p.is_dir()
            and p.name.casefold() in {"assets", "textures"}
            and "scenes" in {part.casefold() for part in p.parts[:-1]}
        ):
...
def build_migration_plan(scan: ProjectScan) -> MigrationPlan:
    ...
    for rel in scan.loose_blends:
        ...
        if rel.parts and rel.parts[0].casefold() == "scenes":
            ...
            ops.append(
                MigrationOperation(
                    source, target, "Loose Blender scene under Scenes", 0.98
                )
            )
            ...
            ops.append(
                MigrationOperation(source, target, "Loose Blender project file", 0.9)
            )
            dirs.add(target.parent)
    for rel in scan.loose_textures:
        ...
        ops.append(
            MigrationOperation(
                source, target, "Loose texture -> shared texture library", 0.98
            )
        )
        dirs.add(target.parent)
    if chars.is_dir():
        for source in chars.glob("*.blend"):
            ...
            ops.append(
                MigrationOperation(
                    source, target, "Loose character file -> character entity", 0.98
                )
            )
            dirs.add(target.parent)
    for rel in scan.uuid_asset_dirs:
        ...
        ops.append(
            MigrationOperation(
                source, target, f"UUID asset folder -> shared {bucket}", 0.92
            )
        )
        dirs.add(target.parent)
...
def apply_migration(
    plan: MigrationPlan, minimum_confidence: float = 0.95
) -> list[MigrationOperation]:
    applied: list[MigrationOperation] = []
    for op in plan.operations:
        if (
            op.confidence < minimum_confidence
            or not op.source.exists()
            or op.destination.exists()
        ):
            continue
        ...
        applied.append(op)
    record_history(
        plan.scan.root, "normalize", plan.scan.profile, f"applied={len(applied)}"
    )
    return applied
...
def generate_metadata(scan: ProjectScan) -> ProjectMetadata:
    code = (
        re.sub(r"[^A-Za-z0-9]+", "_", scan.root.name).strip("_").upper()[:20]
        or "PROJECT"
    )
    return ProjectMetadata(
        project_name=scan.root.name,
        project_code=code,
        pipeline_version="eli_lab",
        project_description=(
            f"Production project discovered by eli_lab. Detected profile: {scan.profile}. "
