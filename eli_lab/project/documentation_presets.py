...
def _entity_lines(entities: list[ProjectEntity]) -> str:
    ...
    for kind in order:
        ...
        sections.append(f"## {kind.replace('_', ' ').title()}s\n\n" + "\n".join(
            f"- **{item.name}** — `{item.relative_path}` ({len(item.files)} files)" for item in items
        ))
...
def build_preset_markdown(summary: WorkspaceSummary, preset: str = "project") -> str:
    ...
    return (f"# {title}\n\n"
            ...
            f"## Project Inventory\n\n{_entity_lines(summary.entities)}\n\n"
            f"## Pipeline\n\nThis project is managed with the ELI LAB production pipeline.\n")


def write_preset_markdown(summary: WorkspaceSummary, output: str | Path, preset: str = "project") -> Path:
