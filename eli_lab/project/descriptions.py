...
def description_seed(entity: ProjectEntity) -> str:
    ...
    return labels.get(entity.kind, "Production entity managed by the eli_lab pipeline.")
...
def build_entity_markdown(entity: ProjectEntity, metadata: EntityMetadata) -> str:
    file_list = (
        "\n".join(f"- `{path.as_posix()}`" for path in entity.files)
        or "- No files discovered"
    )
...
def build_inventory_markdown(summary) -> str:
    ...
    for kind in order:
        ...
        for entity in sorted(items, key=lambda item: item.name.casefold()):
            ...
            lines.append(
                f"- **{metadata.name}** — {metadata.description or description_seed(entity)} (`{entity.path.as_posix()}`)"
            )
        sections.append(f"## {titles[kind]}\n\n" + "\n".join(lines))

    pipeline = (
        "\n".join(f"- `{path.as_posix()}`" for path in summary.files[:50])
        or "- No files discovered"
    )
    return f"""# {name}\n\n{description.strip() or "Production project managed through the eli_lab Multimedia Framework."}\n\n## Project Inventory\n\n{chr(10).join(sections) or "No semantic entities discovered yet."}\n\n## Framework Files\n\n{pipeline}\n"""


def write_inventory_markdown(summary, output: str | Path | None = None) -> Path:
    target = (
        Path(output).expanduser().resolve() if output else summary.root / "README.md"
    )
